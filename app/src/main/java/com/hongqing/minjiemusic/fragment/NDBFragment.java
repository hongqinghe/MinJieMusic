...
/**
 * 使用recycleView来加载布局  由于内容通过网络加载的所以就需要将网络Net
 * Music对象转换为Mp3Info对象 这样在主页中中的播放就可以直接利用原来的播放了
 *
 * Created by 贺红清 on 2017/2/19.
 */

public class NDBFragment extends BaseFragment {
    private View view;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.ndb_fragment_layout, null);
        initView();
        back();
        return view;
    }
    ...
    private void initView() {
        ...
        new JsoupUtils().getNetMusic(getContext(), JsoupUtils.TYPE_NDB, new JsoupUtils.GetNetMusicListener() {
            @Override
            public void onSuccess(final ArrayList<NetMusic> data) {
                setSwipeRefreshLayoutStyle(data);//设置下拉的监听
            ...
            //设置点击事件
                adapter.setOnItemClickListener(new NDB_RecycleViewAdapter.OnItemClickListener() {
                    ...
                    public void onItemClick(View view, int position) {
                        ...
                        EventBus.getDefault().post(new MessageEvent(MessageEventType.PLAY_MUSIC,
                                mp3InfoList, position, Constant.NET_LIST));
                    }
                });
